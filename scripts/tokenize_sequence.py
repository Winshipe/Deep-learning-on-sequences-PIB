...
k = int(snakemake.params[0])#sequence size
sk = int(snakemake.params[1])#token size
outfile = os.path.dirname(snakemake.output[0]) #"tokenized_refined_good.tf" #sys.argv[2]
#line_size = 150
n_ctx = int((sk-1)//2) # the number of nts either side to consider, eg (AA[G]AA)
...
kmers = np.zeros((max_sz,k),dtype='int32')
labels = np.zeros(max_sz, dtype='int32')
...
#        for c in range(min([l,150])):
        for c in range(len(good)):
            tkn_g = 1 # 1 = OOV
            try:
                tkn_g = tokens[good[c:(c+2*n_ctx+1)]]
            except:
                pass
            kmers[r][c] = tkn_g
            labels[r] = 0
        ...
        for c in range(len(bad)):
            tkn_b = 1 # 1 = OOV
            try:
                tkn_b = tokens[bad[c:(c+2*n_ctx+1)]]
            except:
                pass
            labels[r] = 1
            kmers[r][c] = tkn_b
